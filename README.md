...
打开 Mac 的**终端（Terminal）**，完整复制并粘贴以下一行命令，然后按下回车。该命令会自动下载源代码、编译、安装、配置开机自启动，并在完成后自动清理安装包：

```bash
git clone https://github.com/BingkangShi/ExternalDiskTempMonitor.git /tmp/ExternalDiskTempMonitor-install && cd /tmp/ExternalDiskTempMonitor-install && bash install.sh && rm -rf /tmp/ExternalDiskTempMonitor-install
```

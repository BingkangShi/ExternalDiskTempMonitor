...
ExternalDiskTempMonitor 是一款专为 macOS 设计的外置硬盘温度监控工具。它可以实时监控连接到 Mac 的外部硬盘温度，并在屏幕顶部状态栏中显示。
...
- **极简无感监控**：菜单栏只显示一个 macOS 原生的外接硬盘图标（`externaldrive`），实现“0占用”零打扰模式。
  > **💡 操作提示**：只要点击一下状态栏里的“硬盘图标”，即可出现包含所有外置硬盘完整名称和对应实时温度的下拉菜单！
- **详实的状态报告**：下拉菜单会显示所有已连接外置硬盘的**完整卷名称**和实时温度。
...
## 卸载
1. 停止当前运行：
   点击顶部状态栏硬盘图标触发下拉菜单，点击 `Quit ExternalDiskTempMonitor`。
2. 移除开机自启动配置文件（如上）。
